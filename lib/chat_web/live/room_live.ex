defmodule ChatWeb.RoomLive do
  ...
  @impl true
  def handle_event("submite_message", %{"chat"=>%{"message" => message}}, socket) do
  ...
  def  handle_info(%{event: "presence_diff",payload: %{joins: joins,leaves: leaves}}, socket)  do
    join_messages=
      ...
      |> Enum.map(fn username->
        %{type: :system ,uuid: UUID.uuid4(), content: "#{username} joined chat!"}
      end)
    ...
    leave_messages=
      ...
      |> Map.keys()
      |> Enum.map(fn username->
        %{type: :system ,uuid: UUID.uuid4(), content: "#{username} left chat!"}
      end)
